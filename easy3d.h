...
void abre_ventana() {
	...
	ventana.ancho = 640;
	ventana.alto = 480;
	ventana.id = SDL_CreateWindow("Easy3D window", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
							 ventana.ancho, ventana.alto, SDL_WINDOW_OPENGL);	
}

void cierra_ventana() {
	SDL_DestroyWindow(ventana.id);
	SDL_Quit();
}
